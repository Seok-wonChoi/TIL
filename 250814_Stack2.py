...
icp = {'(':3, '*':2, '/':2, '+':1, '-':1}       # 밖에 있을 때의 우선 순위 (클 수록 높음)
isp = {'(':0, '*':2, '/':2, '+':1, '-':1}       # 스택 안에서의 우선 순위 (클 수록 높음)
...
for token in infix:
    if token not in '(+-*/)':        # 피연산자면 후위식에 추가
        ...
        if top == -1 or isp[stack[top]] < icp[token]:      # 토큰의 우선 순위가 더 높음ㄴ
            top += 1 # push
            stack[top] = token
